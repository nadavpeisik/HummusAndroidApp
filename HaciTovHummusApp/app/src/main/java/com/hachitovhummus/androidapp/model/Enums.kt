...
enum class Basic(val type: String, val imageID: Int ,val backgroundColor: Color, val prefix: Char) {
    CHICKPEA("גרגירים", R.drawable.chikpea, Cream3, 'c'),
    TAHINI("טחינה", R.drawable.tahini, Green1,'t'),
    EGG("ביצה", R.drawable.egg, Yellow1,'e'),
    FUL("פול", R.drawable.ful, Brown1, 'f');
}
...
enum class Special(val type: String, val imageID: Int, val gradientColors: List<Color>, val prefix: String, val isSalad: Boolean, val enabledBasics: Set<Basic>, val additionalPrice: Int) {
    ...
    HAMSHUKA("שקשוקה", R.drawable.shakshuka, listOf(Orange1,Orange2), "sh", false, setOf(Basic.CHICKPEA, Basic.TAHINI), 6),
